...
function isZod3Error(error: any): error is z3.ZodError {
  return Array.isArray(error?.issues)
}
function isZod3Schema(schema: any): schema is z3.ZodSchema {
  return (
    '_def' in schema
...
interface RawResolverOptions {
  mode?: 'async' | 'sync'
  raw: true
}
interface NonRawResolverOptions {
  mode?: 'async' | 'sync'
  raw?: false
}
...
export function zodResolver<Input extends FieldValues, Context, Output extends FieldValues>(
